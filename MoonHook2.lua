...
local function updateXrayTransparency(transparency)
...
local function toggleXray()
    ...
    local buildGrates = replicatedStorage:FindFirstChild("BuildGrates")
    if buildGrates then
        for _, folder in pairs(buildGrates:GetChildren()) do
            if folder:IsA("Folder") then
                for _, model in pairs(folder:GetChildren()) do
                    if model:IsA("Model") then
                        for _, part in pairs(model:GetDescendants()) do
                            if part:IsA("BasePart") then
                                if isXrayEnabled and not table.find(xrayParts, part) then
                                    table.insert(xrayParts, part)
                                    part.Transparency = xrayTransparency
                                elseif not isXrayEnabled and table.find(xrayParts, part) then
                                    table.remove(xrayParts, table.find(xrayParts, part))
                                    part.Transparency = 0
                                end
                            end
                        end
...
xrayButton.MouseButton1Click:Connect(function()
    toggleXray()
    xrayToggle.Text = isXrayEnabled and "On" or "Off"
end)
xrayTransparencyButton.MouseButton1Click:Connect(function()
    xrayTransparency = tonumber(xrayTransparencySlider[2].Text) or 0.6
    updateXrayTransparency(xrayTransparency)
end)
...
visualsContent.CanvasSize = UDim2.new(0, 0, 0, yOffsets[1] + 145)
...
local isRangeEnabled = false
local teleportSpeed = 50
local bulletTargets = {}
local quickMoveSpeed = 100 -- Speed for the quick move to Head

yOffsets[5] = yOffsets[5] + 5
local rangeButton = createTextButton(rangeContent, "Range", UDim2.new(0, 10, 0, yOffsets[5]))
local rangeToggle = createTextButton(rangeContent, "Off", UDim2.new(0, 175, 0, yOffsets[5]))
rangeButton.MouseButton1Click:Connect(function()
    isRangeEnabled = not isRangeEnabled
    rangeToggle.Text = isRangeEnabled and "On" or "Off"
end)

yOffsets[5] = yOffsets[5] + 35
local teleportSpeedButton = createTextButton(rangeContent, "Teleport Speed", UDim2.new(0, 10, 0, yOffsets[5]))
local teleportSpeedSlider = createSlider(rangeContent, teleportSpeed, 10, 100, UDim2.new(0, 175, 0, yOffsets[5] + 35))
teleportSpeedButton.MouseButton1Click:Connect(function()
    teleportSpeed = tonumber(teleportSpeedSlider[2].Text) or 50
end)

yOffsets[5] = yOffsets[5] + 70
local recoilButton = createTextButton(rangeContent, "Recoil", UDim2.new(0, 10, 0, yOffsets[5]))
recoilButton.MouseButton1Click:Connect(function()
    -- Delete existing SCAR configuration
    local itemConfigs = replicatedStorage:WaitForChild("ItemConfigs")
    local oldScar = itemConfigs:FindFirstChild("SCAR")
    if oldScar then
        oldScar:Destroy()
    end
    
    -- Create new SCAR configuration
    local scarConfig = Instance.new("ModuleScript")
    scarConfig.Name = "SCAR"
    scarConfig.Parent = itemConfigs
    scarConfig.Source = [[
return {
	Callsign = "SCAR";
	Description = "A quality AR that deals high damage.";
	MaxStack = 1;
	Recipe = {
		Steel = 20;
		Iron = 40;
	...
	CraftAmount = 1;
	Category = "Weapons";
	RequiredSchematic = nil;
	RequiredWorkstation = "AdvancedWorkbench";
	Module = "RangedWeapon";
	HandModel = "SCAR";
	WeldPart = "RightHand";
	TracerPart = "Bullet";
	...
	ProjectileDrop = 4;
	AttackCooldown = 0.116;
	FireAction = "auto";
	AmmoTypes = {"5.56", "AM5.56"};
	ReloadTime = 3;
	EquipTime = 1;
	HipRecoil = {
		rotMag = 0.05;
...
rangeContent.CanvasSize = UDim2.new(0, 0, 0, yOffsets[5] + 105)
...
yOffsets[6] = yOffsets[6] + 70
local freecamButton = createTextButton(playerContent, "Freecam On/Off", UDim2.new(0, 10, 0, yOffsets[6]))
local freecamToggle = createTextButton(playerContent, "Off", UDim2.new(0, 175, 0, yOffsets[6]))
local freecamSpeedButton = createTextButton(playerContent, "Freecam Speed", UDim2.new(0, 10, 0, yOffsets[6] + 35))
local freecamSpeedSlider = createSlider(playerContent, freecamSpeed, 10, 100, UDim2.new(0, 175, 0, yOffsets[6] + 70))
freecamButton.MouseButton1Click:Connect(function()
    isFreecamEnabled = not isFreecamEnabled
    freecamToggle.Text = isFreecamEnabled and "On" or "Off"
    if isFreecamEnabled then
        camera.CameraType = Enum.CameraType.Scriptable
        camera.CFrame = CFrame.new(0, 50, 0) -- Debug camera starts elevated
    else
...
freecamSpeedButton.MouseButton1Click:Connect(function()
    freecamSpeed = tonumber(freecamSpeedSlider[2].Text) or 50
end)

playerContent.CanvasSize = UDim2.new(0, 0, 0, yOffsets[6] + 110)
...
runService.RenderStepped:Connect(function(delta)
        ...
        if userInputService:IsMouseButtonPressed(Enum.UserInputType.MouseButton2) then
            ...
            if target then
                ...
                if head then
                    ...
                    local deltaMove = (targetPos - currentPos) * smoothFactor
                    mousemoverel(deltaMove.X, deltaMove.Y)
                end
            end
        end
    end

    if isRangeEnabled then
        ...
        for _, model in ipairs(workspace:GetChildren()) do
            if model and model:IsA("Model") and model:FindFirstChild("Head") then
                ...
                local headPos, visible = workspace.CurrentCamera:WorldToViewportPoint(head.Position)
                if visible and headPos then
                    local distance = (headPos - mousePos).Magnitude
                    if distance <= circle.Radius and distance < closestDistance then
                        closestDistance = distance
        ...
        if closestTarget and userInputService:IsMouseButtonPressed(Enum.UserInputType.MouseButton2) then
            ...
            if head then
                ...
                local fovCheck = (screenPos - mousePos).Magnitude <= math.tan(math.rad(camera.FieldOfView / 2)) * camera.ViewportSize.Y
                if fovCheck then
                    local ignoreFolder = workspace:FindFirstChild("Ignore")
                    if ignoreFolder then
                        local bullet = ignoreFolder:FindFirstChild("Bullet")
                        if bullet and bullet:IsA("BasePart") then
                            -- Teleport Bullet to Head initially
                            bullet.CFrame = head.CFrame
                            bulletTargets[bullet] = head
                        end
                    end
                end
            end
        ...
        -- Move Bullet toward Head and handle deviation
        for bullet, targetHead in pairs(bulletTargets) do
            if bullet and bullet.Parent and targetHead and targetHead.Parent then
                local direction = (targetHead.Position - bullet.Position).Unit
                local expectedPosition = bullet.Position + direction * teleportSpeed * delta
                local actualVelocity = bullet.Velocity

                -- Check if bullet is moving off-script (simplified check)
                if actualVelocity.Magnitude > 0 and (actualVelocity.Unit - direction).Magnitude > 0.1 then
                    -- Stop the bullet
                    bullet.Velocity = Vector3.new(0, 0, 0)
                    -- Teleport near the Head (e.g., 2 studs away)
                    local nearPosition = targetHead.Position + (direction * 2)
                    bullet.Position = nearPosition
                    -- Quick move to Head
                    local quickDirection = (targetHead.Position - bullet.Position).Unit
                    bullet.Position = bullet.Position + quickDirection * quickMoveSpeed * delta
                    bullet.Velocity = quickDirection * quickMoveSpeed
                else
                    -- Normal movement
                    local moveDistance = teleportSpeed * delta
                    bullet.Position = bullet.Position + direction * moveDistance
                    bullet.Velocity = direction * teleportSpeed
                end

                -- Update target if bullet gets too far
                if (bullet.Position - targetHead.Position).Magnitude > 10 then
                    bullet.CFrame = targetHead.CFrame
                end
            else
                bulletTargets[bullet] = nil
            end
        end
    end
    ...
    if isFreecamEnabled then
        ...
        moveDirection = moveDirection.Unit * freecamSpeed * delta
        camera.CFrame = camera.CFrame + moveDirection

        local mouseDelta = userInputService:GetMouseDelta()
        local sensitivity = 0.2
        camera.CFrame = camera.CFrame * CFrame.Angles(0, -mouseDelta.X * sensitivity * delta, 0) * CFrame.Angles(-mouseDelta.Y * sensitivity * delta, 0, 0)
    end
...
-- Monitor Bullet creation in workspace.Ignore
workspace.Ignore.ChildAdded:Connect(function(child)
    if child.Name == "Bullet" and child:IsA("BasePart") then
        if isRangeEnabled and userInputService:IsMouseButtonPressed(Enum.UserInputType.MouseButton2) then
            local target = findClosestPlayerInCircle()
            if target then
                local head = target:FindFirstChild("Head")
                if head then
                    ...
                    local fovCheck = (screenPos - mousePos).Magnitude <= math.tan(math.rad(camera.FieldOfView / 2)) * camera.ViewportSize.Y
                    if fovCheck then
                        child.CFrame = head.CFrame
                        bulletTargets[child] = head
                    end
                end
            end
        end
...
workspace.Ignore.ChildRemoved:Connect(function(child)
    if child.Name == "Bullet" then
        bulletTargets[child] = nil
    end
end)
